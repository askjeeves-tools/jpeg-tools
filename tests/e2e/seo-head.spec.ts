...
test("homepage has SEO meta tags and structured data", async ({ page }) => {
	...
	await expect(expander.locator("summary")).toHaveText("Supported JPEG conversions");
	await expect(expander.locator("li")).toHaveCount(7);
	expect(await page.content()).toContain("JPEG → PNG");

	await expect(page.locator(".seo-faq-item")).toHaveCount(7);
	...
	await expect(
		seoExpanders.filter({
			has: page.locator("summary", { hasText: "Supported JPEG conversions" }),
		}),
	).toBeVisible();
	await expect(
...
test("static SEO files are served", async ({ request }) => {
	...
	expect(robotsText).toContain("Sitemap: https://jpeg.askjeeves.cc/sitemap-index.xml");
