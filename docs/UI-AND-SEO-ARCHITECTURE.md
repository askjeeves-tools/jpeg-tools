# jpeg-tools UI & SEO Architecture Guide

Reference implementation for Ask Jeeves single-page converter sites. This repo follows the same architecture as `png-tools`; use this document when scaffolding another sibling tool (e.g. `webp.askjeeves.cc`, `pdf.askjeeves.cc`).
...
`buildJpegToolsExtraGraph(siteOrigin, featureList)` returns 5 linked `@graph` entities:
...
8. **`src/seo-structured-data.ts`** — copy `buildJpegToolsExtraGraph` pattern from this repo; adjust `applicationSubCategory`, HowTo name for new tools
